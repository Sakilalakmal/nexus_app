...
import { CollapsibleChannelList } from "./_components/ChannelList";
import { WorkspaceMembersList } from "./_components/WorkspaceMemberList";
import { getQueryClient, HydrateClient } from "@/lib/query/hydration";
import { orpc } from "@/lib/orpc";

async function ChannelListLayout({ children }: { children: React.ReactNode }) {
    ...
    <>
      <div className="flex h-full w-80 flex-col bg-secondary border-r border-border">
        ...
        <div className="px-4 py-2 border-t border-border">
          <Collapsible defaultOpen>
            <CollapsibleTrigger className="flex w-full items-center justify-between px-2 py-1 text-sm font-medium text-muted-foreground hover:text-foreground [&[data-state=open]>svg]:rotate-180">
              members
